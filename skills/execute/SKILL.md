...
- **A task would restrict or remove existing behavior** - see below

**Ask for clarification rather than guessing.** Don't force through blockers.

### The restriction gate

Before writing any code that rejects a combination, locks a control, narrows what a role may do, or removes behavior already on the branch: **stop and ask.**

This is a hard gate, not a judgement call, because the failure is invisible - a guard that shouldn't exist looks exactly like a guard that should, and it passes its own tests. Two questions settle it:

1. **Is the requirement a system constraint or a process expectation?** "They can't do X while doing Y" may mean the code rejects it, or that agents are trained not to. Tells that it is training: it is addressed to a person to communicate, it says "expectation" rather than naming an error, or it describes what someone *should* do rather than what the system does.
2. **What breaks?** Name the existing flow that stops working and the role that loses an ability. If you can't name one, the restriction is probably unnecessary. If you can, that sentence goes to the user before the code gets written.

`SHARED.md` ("Implementing from a ticket: ask before you restrict") has the full rule.
