...
- **One caller is not a smell - when it stays on the entity.** A query or mutation method with a single caller is fine as a model or queryset method. It is not fine as a free function in another module: **if extracting it adds an import to the caller, inline it instead.** A `has_agent_added_unit_security_fee(quote)` parked in `quotes/utils`, called only by a `LeaseTransaction` property that already imported the model and the constant, should have stayed a four-line queryset expression in the property.
...
- **Extract when a function is reused or answers a named question - never to hit a line count.** A helper called once, from one place, belongs inlined in its caller. (This does not contradict "one caller is not a smell" above: a method on the entity it reads is fine, a free function in another module is not.)
